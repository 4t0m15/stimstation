...
impl Drawer for Renderer {
    ...
    fn draw_filled_circle(&self, frame: &mut [u8], width: u32, height: u32, 
                         center_x: i32, center_y: i32, radius: i32, 
                         color: &[u8; 4], x_offset: usize, buffer_width: u32) {
        draw_filled_circle_internal(frame, width, height, center_x, center_y, radius, color, x_offset, buffer_width);
    }
    
    fn draw_shadow_glow(&self, frame: &mut [u8], width: u32, height: u32,
                       center_x: i32, center_y: i32, radius: i32, 
                       color: &[u8; 4], x_offset: usize, buffer_width: u32) {
        draw_shadow_glow_internal(frame, width, height, center_x, center_y, radius, color, x_offset, buffer_width);
    }
}
...
fn draw_filled_circle_internal(frame: &mut [u8], width: u32, height: u32, 
                              center_x: i32, center_y: i32, radius: i32, 
                              color: &[u8; 4], x_offset: usize, buffer_width: u32) {
...
fn draw_shadow_glow_internal(frame: &mut [u8], width: u32, height: u32,
                            center_x: i32, center_y: i32, radius: i32, 
                            color: &[u8; 4], x_offset: usize, buffer_width: u32) {
    for y in -radius..=radius {
        for x in -radius..=radius {
            ...
            if dist_sq <= radius * radius {
                ...
                if pixel_x >= 0 && pixel_x < width as i32 && pixel_y >= 0 && pixel_y < height as i32 {
                    let idx = 4 * ((pixel_y as usize * buffer_width as usize) + pixel_x as usize + x_offset);
...
pub fn draw_shadow_glow(frame: &mut [u8], width: u32, height: u32,
                       center_x: i32, center_y: i32, radius: i32, 
                       color: &[u8; 4], x_offset: usize, buffer_width: u32) {
    draw_shadow_glow_internal(frame, width, height, center_x, center_y, radius, color, x_offset, buffer_width);
}

fn put_pixel(frame: &mut [u8], width: u32, height: u32, x: i32, y: i32, 
            color: &[u8; 4], x_offset: usize, buffer_width: u32) {
...
pub fn draw_rays_from_ball(frame: &mut [u8], width: u32, height: u32, pos: (f32, f32), 
    ...
    for i in 0..count {
        ...
        if discriminant >= 0.0 {
            ...
            if (t1 > 0.0 && t1 < ray_length) || (t2 > 0.0 && t2 < ray_length) {
                ...
                draw_line_internal(frame, width, height, source_x, source_y, intersect_x, intersect_y, 
    ...
    for shadow in shadow_rays {
        draw_line_internal(frame, width, height, 
