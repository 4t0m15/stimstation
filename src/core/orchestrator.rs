use crate::{physics, graphics::render, algorithms::sorter_manager, integration};
...
fn draw_balls_and_rays(frame: &mut [u8], width: u32, height: u32, time: f32, 
                      scale_x: f32, scale_y: f32, x_offset: usize, buffer_width: u32) {    let (yellow_pos, green_pos) = physics::physics::get_ball_positions();
    
    if let (Some(yellow_pos), Some(green_pos)) = (yellow_pos, green_pos) {
        let draw_rays_closure = |frame: &mut [u8], width: u32, height: u32, pos: (f32, f32), ray_color: [u8; 4], time: f32, x_offset: usize, buffer_width: u32| {
            let other_pos = if pos == yellow_pos { green_pos } else { yellow_pos };
            render::draw_rays_from_ball(frame, width, height, pos, ray_color, time, x_offset, buffer_width, other_pos);
        };
        
        physics::physics::draw_balls_with_effects(frame, width, height, time, scale_x, scale_y, x_offset, buffer_width, draw_rays_closure);
    }
}
