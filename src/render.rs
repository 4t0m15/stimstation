use crate::sorter::{SortVisualizer, SortState, SortAlgorithm};

fn draw_line_internal(frame: &mut [u8], width: u32, height: u32, 
...
pub fn draw_line(frame: &mut [u8], width: u32, height: u32, 
                x0: i32, y0: i32, x1: i32, y1: i32, 
                color: &[u8; 4], x_offset: usize, buffer_width: u32) {
    draw_line_internal(frame, width, height, x0, y0, x1, y1, color, x_offset, buffer_width);
}

pub fn draw_filled_circle(frame: &mut [u8], width: u32, height: u32, 
                         center_x: i32, center_y: i32, radius: i32, 
                         color: &[u8; 4], x_offset: usize, buffer_width: u32) {

pub trait Drawer {
    fn draw_line(&self, frame: &mut [u8], width: u32, height: u32, 
                x0: i32, y0: i32, x1: i32, y1: i32, 
                color: &[u8; 4], x_offset: usize, buffer_width: u32);
    
    fn draw_filled_circle(&self, frame: &mut [u8], width: u32, height: u32, 
                         center_x: i32, center_y: i32, radius: i32, 
                         color: &[u8; 4], x_offset: usize, buffer_width: u32);
    
    fn draw_shadow_glow(&self, frame: &mut [u8], width: u32, height: u32,
                       center_x: i32, center_y: i32, radius: i32, 
                       color: &[u8; 4], x_offset: usize, buffer_width: u32);
}
...
impl Drawer for Renderer {
    ...
    fn draw_shadow_glow(&self, frame: &mut [u8], width: u32, height: u32,
        ...
        let inner_radius_sq = (radius - 5).max(0) * (radius - 5).max(0);
            ...
            for i in 0..n {
                ...
                for j in 0..bar_height {
                    for k in 0..(bar_width as usize) {
                        put_pixel_safe(frame, (x + (i as f32 * bar_width) as usize + k) as i32, 
                                     (y + height - j - 1) as i32, buffer_width, height as u32, 
                                     [color[0], color[1], color[2], 255], x_offset);
                    }
                }
            }
        } else {
            ...
            for i in 0..n {
                ...
                for j in 0..bar_width {
                    for k in 0..(bar_height as usize) {
                        put_pixel_safe(frame, (x + j) as i32, 
                                     (y + (i as f32 * bar_height) as usize + k) as i32, 
                                     buffer_width, height as u32, 
                                     [color[0], color[1], color[2], 255], x_offset);
                    }
                }
            }
...
pub fn draw_rays(frame: &mut [u8], width: u32, height: u32, 
            source_x: i32, source_y: i32, 
            center_x: i32, center_y: i32,
            radius: i32, color: &[u8; 4], 
            count: usize, time: f32, 
            x_offset: usize, buffer_width: u32,
            yellow_pos: Option<(f32, f32)>, green_pos: Option<(f32, f32)>) {
    let (other_x, other_y, other_radius) = if source_x == yellow_pos.unwrap_or((0.0, 0.0)).0 as i32 && 
    ...
    for i in 0..count {
        ...
        if discriminant >= 0.0 {
            ...
            if (t1 > 0.0 && t1 < ray_length) || (t2 > 0.0 && t2 < ray_length) {
                ...
                let intersect_y = (source_y as f32 + ray_dir_y * t) as i32;                self.draw_line(frame, width, height, source_x, source_y, intersect_x, intersect_y, 
                ...
                draw_line_internal(frame, width, height, source_x, source_y, end_x as i32, end_y as i32, 
                         color, x_offset, buffer_width);
            }
        } else {
            draw_line_internal(frame, width, height, source_x, source_y, end_x as i32, end_y as i32, 
    ...
    for shadow in shadow_rays {        draw_line_internal(frame, width, height, 
...
pub fn draw_rays_from_ball(frame: &mut [u8], width: u32, height: u32, pos: (f32, f32), 
                          ray_color: [u8; 4], time: f32, x_offset: usize, buffer_width: u32,
                          other_pos: (f32, f32)) {
    ...
    for i in 0..count {
        ...
        if discriminant >= 0.0 {
            ...
            if (t1 > 0.0 && t1 < ray_length) || (t2 > 0.0 && t2 < ray_length) {
                let t = t1.max(0.0);
                let intersect_x = (source_x as f32 + ray_dir_x * t) as i32;
                let intersect_y = (source_y as f32 + ray_dir_y * t) as i32;                draw_line_internal(frame, width, height, source_x, source_y, intersect_x, intersect_y, 
                         &ray_color, x_offset, buffer_width);
